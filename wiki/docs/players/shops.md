...
1. Place a **sign** on a container (chest, barrel, etc.) inside your stall.
2. Line 1: `[em]` (the trigger token).
3. Line 2: **Quantity** per trade (e.g., `64`).
4. Line 3: **Price** in server currency (e.g., `100`).
5. The sign auto-formats into a working shop.

## Trade directions

| Direction | What happens | Sign line 3 |
|-----------|-------------|-------------|
| **SELL** (default) | You sell items to players. They pay you, receive items from your container. | Price (e.g., `100`) |
| **BUY** | You buy items from players. They give you items, receive payment. | Price (e.g., `100`) |
| **TRADE** | Barter: players give you one item, receive another. | `10xdiamond` (what you pay) |
